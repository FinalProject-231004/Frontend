...
import { CustomizedButtons, UserInfoInput } from '@/components/common';
...
export default function VerifyPassword() {
  ...
  const success = () => toast.success('인증 성공!!', {
    position: "top-center",
    autoClose: 2000,
  ...
  const wrongPw = () => toast.error('비밀번호가 일치하지 않습니다.', {
    position: "top-center",
    autoClose: 2000,
    hideProgressBar: true,
    closeOnClick: false,
        ...
        <div className='flex flex-col justify-center items-center gap-10 '>
          <div className='w-full py-16 px-10 border-t-2 border-b flex flex-col justify-center items-center gap-9'>
            <div className='w-[700px] flex justify-between items-center'>
              ...
              <UserInfoInput inputVal={nickName} onChange={()=>{}} 
              type='text' placeholder='아이디' size='medium' borderColor={''} focusBorderColor={''} />
            </div>
            <div  className='w-[700px] flex justify-between items-center'>
              <p className='w-full text-2xl'>비밀번호</p> 
              <UserInfoInput inputVal={pwInput} onChange={(e)=>{setPwInput(e.target.value)}} 
              type='password' placeholder='비밀번호' size='medium' borderColor={''} focusBorderColor={''} />
            </div>
          </div>
          <CustomizedButtons size='small' fontColor='white' fontSize='24px' BtnName='확인' BtnBg='blue' BtnHoverBg={''} BtnActiveBg={''} borderRadius='6px' 
          onClick={() => {
