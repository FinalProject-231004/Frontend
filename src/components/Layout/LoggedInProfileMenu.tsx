...
import { LoggedInAttendence } from '.';
import { getAPI } from '@/apis/axios';
import { profileAPIResponse } from '@/types/header';
import { useState } from 'react';
import { logOut } from '@/utils/authHelpers';
import { userMileageState, userNickNameState } from '@/recoil/atoms/userInfoAtom';
...
export default function LoggedInProfileMenu() {
  ...
  const getUserInfo =async () => {
    try {
      const response = await getAPI<profileAPIResponse>('/api/mypage/memberInfo');
          ...
          <Box sx={{ flexGrow: 0 }}>
            {/* <Tooltip title="Open settings"> */}
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="/static/images/avatar/2.jpg" />  {/* image */}
              </IconButton>
            {/* </Tooltip> */}
            <Menu
              sx={{ mt: '45px' }}
