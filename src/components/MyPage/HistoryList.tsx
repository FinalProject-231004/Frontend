...
export default function HistoryList({price, cost, itemName, quantity, email, date}:historyListProps) {
  ...
  return (
    <div className='w-[988px] h-[70px] py-[23px] pl-[25px] pr-[14px] border-b border-blue flex justify-around items-center'>
      {price === 'reward' ? (
        <p className='text-[22px] text-blue ' style={defaultStyle}>+{cost}M</p>
      ) 
      : (
        <p className='text-[22px] text-[#FF3D00]' style={defaultStyle}>{cost}M</p>
      )}
