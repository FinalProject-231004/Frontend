...
export default function ChangeNicknameModal() {
  ...
  return (
    <>
      <button onClick={newNicknameModal.open} className='bg-blue rounded-r-md text-[24px] text-white hover:bg-navy w-[131px] h-[72px]'>변경하기</button>
      <Modal onRequestClose={closeModal} width='713px' height='368px' bgColor='#F1F8FF' isOpen={newNicknameModal.isOpen}>
        <div className='h-[368px] flex flex-col justify-center items-center'>
          ...
          <div className='relative'>
            <div className='w-[530px] relative flex justify-center items-center z-10'>
              <UserInfoInput inputVal={newNickname} type='text' placeholder='아이디' size='medium' borderColor='blue' focusBorderColor={''}
              onChange={(e)=>nicknameOnChange(e)} />
              <button className='w-[131px] h-[72px] rounded-r-[6px] bg-[#3E3E3E] text-white text-[24px] absolute z-20 right-0'
              onClick={()=>{duplicateVerify(updateNickname);}}>
                중복확인
              </button>
            </div>
            {newNickname.length > 0 && <div className='my-[6px] absolute text-[16px] text-[#F92316] font-hairline'>{conditionMsg}</div>}
            <div className='my-[6px] absolute text-[16px] right-0 font-hairline' 
            // style={{ color: isNicknameAvailable === true ? 'blue' : isNicknameAvailable === false ? '#0078FF' : '#F92316' }}
            >{duplicateMsg}</div>
          </div>

          <div className='mt-[60px]'>
            <CustomizedButtons size='large' fontColor='white' fontSize='24px' BtnName='저장하기' BtnBg='#0078FF' BtnHoverBg={''} BtnActiveBg={''} borderRadius='28.5px' 
            onClick={()=>{putNickname(updateNickname);}} 
            />
          </div>
        </div>
      </Modal>
